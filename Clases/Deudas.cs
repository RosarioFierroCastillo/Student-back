...
namespace API_Archivo.Clases
{
...

    {
        ...
        */
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        Deuda.Add(new Deudoress()
                        {
                            ...
                            proximo_pago = !reader.IsDBNull(11) ? (reader.GetDateTime(11)).ToString("yyyy-MM-ddTHH:mm:ss") : "",
        ...
        public bool AsignarDeudaNuevaATodos(int id_fraccionamiento, int destinatario)
        {
            List<Deudas> Deuda = new List<Deudas>();
            ...
            using (MySqlConnection conexion = new MySqlConnection(Global.cadena_conexion))
            {
                ...
                if (isindividual)
                {
                    comando.Parameters.Add("@id_persona", MySqlDbType.Int32).Value = destinatario;
                }
            ...
            using (MySqlConnection conexion = new MySqlConnection(Global.cadena_conexion))
            {
                ...
                foreach (Personas persona in Persona)
                {
                    MySqlCommand insertDeudor = new MySqlCommand(
                        "INSERT INTO deudores (id_deudor,id_deuda, id_fraccionamiento,nombre_persona, lote, tipo_deuda, nombre_deuda, monto,recargo,dias_gracia, proximo_pago, estado,periodicidad, monto_pendiente, comprobante) " +
                        "VALUES (@id_deudor,@id_deuda, @id_fraccionamiento,@nombre_persona, @lote, @tipo_deuda, @nombre_deuda, @monto,@recargo,@dias_gracia, @proximo_pago, @estado,@periodicidad, @monto, @comprobante)", conexion);
                    ...
                    insertDeudor.Parameters.AddWithValue("@comprobante", "n");
